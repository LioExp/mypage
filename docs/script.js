...
const t = {
  pt: {
    ...
    about: { heading: ['Eu construo ', 'sistemas', '.'], displayTitle: ['about', 'me'], cbpText: 'Create. Break. Protect. Não como etapas separadas, mas como um ciclo.', fullText: `<span class="about-text-line">Não só para fazer funcionar. Quero entender por que funciona, quanto custa e como pode ser quebrado. Sou builder e programador, e é isso que me move: descer abaixo da abstração, construir as minhas próprias soluções e descobrir até onde aguentam.</span><span class="about-text-line">É assim que aprendo: construindo. Python, Rust, Linux, networking, segurança e IA não são uma lista de tecnologias — são ferramentas diferentes para problemas diferentes. Alguns projetos começam como uma pergunta, outros como um problema real. O que sai disso é código, experimento, ferramenta e, principalmente, coisas que posso testar no mundo real.</span><span class="about-text-line">Mas construir é só o começo.</span><span class="about-text-line">Depois quero saber onde quebra, quanto consome, quais são os limites, o que acontece quando alguém usa de uma forma que eu não esperava. Daí vem a minha forma de trabalhar: Create. Break. Protect.</span><span class="about-text-line">Segurança, performance e eficiência não entram no fim — fazem parte da construção desde o início.</span><span class="about-text-line">E essa forma de trabalhar nasceu de um contexto específico.</span><span class="about-text-line">Construo em Luanda <a class="about-inline-link" href="https://www.google.com/maps/search/?api=1&query=Luanda%2C%20Angola" target="_blank" rel="noopener noreferrer" aria-label="Abrir Luanda no mapa" title="Abrir Luanda no mapa">${ABOUT_ICONS.pin}</a>, com hardware modesto <span class="about-inline-icon" aria-hidden="true">${ABOUT_ICONS.monitor}</span> e uma conexão que nem sempre colabora. Isso ensinou-me a prestar atenção ao que é fácil ignorar quando há recursos de sobra: RAM, CPU, armazenamento, latência, dependências, consumo.</span><span class="about-text-line">Quando os recursos são limitados, cada decisão importa. Deixei de ver isso apenas como limitação e passei a ver como parte do problema de engenharia.</span><span class="about-text-line">É esse problema que continuo a perseguir.</span><span class="about-text-line">Ainda estou construindo o caminho, e a direção é clara: entender sistemas cada vez mais profundamente e levar isso para a interseção entre <a class="about-inline-link" href="#projectos" aria-label="Ver o trabalho construído" title="Ver o trabalho construído">segurança e inteligência artificial ${ABOUT_ICONS.palette}</a>.</span><span class="about-text-line">Systems → Linux → Networking → Security → AI Security.</span><span class="about-text-line">Não é uma lista para aprender. São camadas que preciso de entender para chegar onde quero.</span><span class="about-text-line">Ainda há muito para construir, quebrar e aprender.</span><span class="about-text-line"><a class="about-inline-link" href="#projectos" aria-label="Ver roadmap" title="Ver roadmap">→ ver roadmap</a></span>`, openTags: ['collab', 'open source'] },
    projects: {
  ...
  en: {
    ...
    about: { label: 'ABOUT', heading: ['I build ', 'systems', '.'], displayTitle: ['about', 'me'], cbpText: 'Create. Break. Protect. Not as separate steps, but as a cycle.', fullText: `<span class='about-text-line'>Not just to make things work. I want to understand why they work, how much they cost, and how they can break. I'm a builder and programmer, and that's what drives me — going below the surface, building my own solutions, and discovering how far they hold.</span><span class='about-text-line'>That's how I learn: by building. Python, Rust, Linux, networking, security, and AI aren't a technology list — they're different tools for different problems. Some projects start as a question, others as a real problem. What comes out is code, experiment, tool, and most importantly, something I can test in the real world.</span><span class='about-text-line'>But building is just the beginning.</span><span class='about-text-line'>Then I want to know where it breaks, how much it consumes, what the limits are, what happens when someone uses it in a way I didn't expect. That's where my way of working comes from: Create. Break. Protect.</span><span class='about-text-line'>Security, performance, and efficiency don't come in at the end — they're part of the build from the start.</span><span class='about-text-line'>And this way of working was born from a specific context.</span><span class='about-text-line'>I build in Luanda <a class='about-inline-link' href='https://www.google.com/maps/search/?api=1&query=Luanda%2C%20Angola' target='_blank' rel='noopener noreferrer' aria-label='Open Luanda in Maps' title='Open Luanda in Maps'>${ABOUT_ICONS.pin}</a>, with modest hardware <span class='about-inline-icon' aria-hidden='true'>${ABOUT_ICONS.monitor}</span> and a connection that doesn't always cooperate. That taught me to pay attention to what's easy to ignore when you have resources to spare: RAM, CPU, storage, latency, dependencies, usage.</span><span class='about-text-line'>When resources are limited, every decision matters. I stopped seeing this only as a limitation and started seeing it as part of the engineering problem.</span><span class='about-text-line'>That's the problem I keep pursuing.</span><span class='about-text-line'>I'm still building the path, and the direction is clear: understand systems more deeply and take that to the intersection between <a class='about-inline-link' href='#projectos' aria-label='View the work' title='View the work'>security and artificial intelligence ${ABOUT_ICONS.palette}</a>.</span><span class='about-text-line'>Systems → Linux → Networking → Security → AI Security.</span><span class='about-text-line'>It's not a list to learn. It's layers I need to understand to get where I want to go.</span><span class='about-text-line'>There's still a lot to build, break, and learn.</span><span class='about-text-line'><a class='about-inline-link' href='#projectos' aria-label='View roadmap' title='View roadmap'>→ view roadmap</a></span>`, openTags: ['collab', 'open source'] },
    projects: {
