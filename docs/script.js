  ...
  $('openTags').innerHTML = data.about.openTags.map(t => `<span class="tag">${t}</span>`).join('');
    ...
    const onScroll = () => {
      ...
      if (progress === 0) {
        ...
        aboutFooter.scrollTop = Math.max(0, Math.min(maxScroll, lineCenter - readingLine));
      }
    };
