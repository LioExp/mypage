...
const t = {
  pt: {
    ...
    projects: {
      ...
      myroadmapp: { badge: 'activo', desc: 'Gerador de roadmaps profissionais a partir de ideias simples. O utilizador escreve o que quer e a plataforma gera um plano visual.', problem: 'O problema', problemText: 'A maioria das pessoas tem ideias mas não consegue transformá-las num plano de acção claro. Ferramentas de project management são demasiado complexas para quem está a começar, e as soluções simples não geram estrutura.', how: 'Como funciona', howText: 'O utilizador insere uma ideia e o sistema gera automaticamente um roadmap visual com fases, marcos e dependências. Interface limpa, sem necessidade de conta para começar.', features: 'Funcionalidades', featureItems: ['Geração automática de roadmap a partir de texto', 'Interface visual com fases e marcos', 'Exportação e partilha de planos', 'Sem necessidade de registo para começar', 'Design responsivo para mobile e desktop'], vision: 'Visão', visionText: 'Tornar o planeamento acessível para qualquer pessoa em qualquer lugar, independentemente do hardware ou ligação à internet.' },
  ...
  en: {
    ...
    projects: {
      ...
      myroadmapp: { badge: 'active', desc: 'Professional roadmap generator from simple ideas. The user writes what they want and the platform generates a visual plan.', problem: 'The problem', problemText: 'Most people have ideas but can\'t turn them into a clear action plan. Project management tools are too complex for beginners, and simple solutions don\'t generate structure.', how: 'How it works', howText: 'The user enters an idea and the system automatically generates a visual roadmap with phases, milestones and dependencies. Clean interface, no account needed to start.', features: 'Features', featureItems: ['Automatic roadmap generation from text', 'Visual interface with phases and milestones', 'Export and share plans', 'No registration required to start', 'Responsive design for mobile and desktop'], vision: 'Vision', visionText: 'Make planning accessible to anyone anywhere, regardless of hardware or internet connection.' },
